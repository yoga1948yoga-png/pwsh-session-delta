...
$result=Invoke-Pester -Path (Join-Path $PSScriptRoot 'Compare.Tests.ps1') -Output Detailed -PassThru
if ($ResultPath) {
    $report=[ordered]@{
        stage='2B';powerShell=$PSVersionTable.PSVersion.ToString();pester=(Get-Module Pester).Version.ToString()
